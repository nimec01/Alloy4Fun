...
applyThemeSettings = function () {
    ...
    if (atomSettings.unconnectedNodes) for (let i = 0; i < atomSettings.unconnectedNodes.length; i++) setUnconnectedNodesValue(atomSettings.unconnectedNodes[i].type, atomSettings.unconnectedNodes[i].unconnectedNodes);
        ...
        instance.fields.forEach((field) => {
            if (field.type.indexOf('this/') != -1) {
                field.values.forEach((relation) => {
                    ...
                    const labelExt = relation.splice(1, relation.length - 2).toString();
                    result.push({
                        group: 'edges',
                        selectable: true,
                        data: {
        ...
        style: [ // the stylesheet for the graph
            {
                ...
                style: {
                    ...
                    label(ele) {
                        ...
                        let attributes = '';
                        for (const i in ele.data().attributes) {
