...
public class PolicyManager {
    ...
    public void computePolicyForGraph(ObjectId graph_id, PolicyRule costPolicy) {
        Graph.removeAllPolicyStats(graph_id);
        clearPolicy(graph_id);
        costPolicy.normalizeByGraph(graph_id);
        long t = System.nanoTime();
        djisktra(graph_id, costPolicy);
    ...
    public void djisktra(ObjectId graph_id, PolicyRule r) {
        Collection<DjisktraNode> batch = nodeRepo.streamByGraphIdAndValid(graph_id).map(DjisktraNode::new).toList();
        while (!batch.isEmpty()) {
            ...
            batch = nodes.get(true)
                    ...
                    .flatMap(n -> edgeRepo.streamTransitionsByDestinationScoreGT(n.node, min).parallel().map(t -> n.apply(t, r)))
                    .collect(Collectors.toConcurrentMap(x -> x.node.id, x -> x, Ordered::min, () -> map)).values();
        }
    }
